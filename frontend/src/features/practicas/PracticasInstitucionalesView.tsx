...
const PROCESS_OPTIONS: Array<{ code: PracticasProcessCode; label: string; short: string }> = [
  { code: 'PPF', label: 'Prácticas laborales', short: 'Prácticas laborales' },
  { code: 'VIN', label: 'Servicio Comunitario', short: 'Servicio Comunitario' },
]
...
function normalizeCareerText(value: unknown) {
  return String(value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .trim()
...
function isExcludedProcessCareer(item: PracticasEligibilityItem) {
  const careerCode = String(item.CodigoCarrera || '').trim()
  const careerName = normalizeCareerText(item.Carrera)
  return careerCode === '12' || careerName.includes('INGL')
}

function uniqueProcessStudents(items: PracticasEligibilityItem[]) {
  const seen = new Set<string>()
  const result: PracticasEligibilityItem[] = []
  for (const item of items) {
    if (isExcludedProcessCareer(item)) continue
    const code = Number(item.codigo_estud)
    const key = code ? String(code) : `${item.Cedula_Est || ''}|${item.CodigoCarrera || ''}`
...
export function PracticasInstitucionalesView({
  ...
  const periodEnrollmentStudents = useMemo(
    () => uniqueProcessStudents(adminElegibles),
    [adminElegibles]
  )

  const periodEnrollmentCodes = useMemo(
    () => new Set(periodEnrollmentStudents.map((item) => Number(item.codigo_estud)).filter(Boolean)),
    [periodEnrollmentStudents]
  )

  const validSelectedStudents = useMemo(
    () => selectedStudents.filter((code) => periodEnrollmentCodes.has(code)),
    [periodEnrollmentCodes, selectedStudents]
  )

  const selectablePeriodEnrollmentCount = useMemo(
    () => periodEnrollmentStudents.filter((item) => Boolean(item.PuedeMatricular)).length,
    [periodEnrollmentStudents]
  )

  const excludedCareerRows = useMemo(
    () => adminElegibles.filter((item) => isExcludedProcessCareer(item)).length,
    [adminElegibles]
  )

  const duplicatedRows = Math.max(adminElegibles.length - excludedCareerRows - periodEnrollmentStudents.length, 0)
  ...
  function toggleStudent(code: number) {
    const student = periodEnrollmentStudents.find((item) => Number(item.codigo_estud) === code)
  ...
  function toggleAllStudents() {
    const codes = periodEnrollmentStudents
      .filter((item) => Boolean(item.PuedeMatricular))
      .map((item) => Number(item.codigo_estud))
      .filter(Boolean)
    setSelectedStudents(() => validSelectedStudents.length === codes.length ? [] : codes)
  }
  ...
  async function loadAll() {
    ...
    try {
      ...
      setError(apiError instanceof Error ? apiError.message : 'No se pudo cargar prácticas laborales y Servicio Comunitario.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function saveResponsable() {
    ...
    if (!selectedPeriod) {
      setError('Selecciona el nuevo periodo del proceso donde se va a matricular.')
      return
    }
    ...
    if (!validSelectedStudents.length) {
    ...
    try {
      const response = await savePracticasPeriodoDesignacion({
        ...
        rol_responsable: 'RESPONSABLE',
        estudiantes: validSelectedStudents,
      })
      setResponsableForm({
        nombre_responsable: '',
        cedula_responsable: '',
      ...
      setTeacherSearch('')
      setSelectedStudents([])
      setMessage(String(response.message || 'Matrícula por periodo registrada correctamente.'))
      await loadCatalog()
      await loadAdmin()
      await loadStudent()
      await loadPeriodDesignations()
  ...
  return (
    <section className="portal-student-page practicas-page">
      <header className="portal-student-hero practicas-hero">
        <small>Prácticas laborales</small>
        <h1>Prácticas laborales y Servicio Comunitario</h1>
      ...
      {mode === 'responsable' || isResponsible ? (
        <section className="student-card student-card--wide matricula-panel">
          <div className="section-title">
            ...
            <strong>Avance de prácticas laborales</strong>
          </div>
        ...
        <section className="student-card student-card--wide matricula-panel">
          <div className="section-title">
            ...
            <strong>{isAdmin ? 'Matrícula administrativa' : 'Mis procesos asignados'}</strong>
          </div>
          {isAdmin ? (
            ...
            <p className="portal-muted">
              La matrícula de Prácticas laborales y Servicio Comunitario la realiza administración. Desde aquí puedes descargar y subir la carta compromiso cuando tengas un expediente asignado.
            </p>
          )}
        ...
        <section className="student-card student-card--wide matricula-panel practicas-admin-card">
          ...
          <p className="portal-muted">
            Selecciona primero el periodo académico donde está el estudiante, luego el periodo destino del proceso donde quedará matriculado. Todo se guarda únicamente en la base del proceso.
          </p>
          ...
          <div className="matricula-acad-form practicas-form practicas-responsable-form">
            ...
            <label>
              <span>Periodo destino del proceso ({periodos.length} periodo(s))</span>
              <select
          ...
          <div className="practicas-student-picker">
            <div className="practicas-picker-head">
              ...
              <button type="button" className="ghost-button" onClick={toggleAllStudents} disabled={!periodEnrollmentStudents.length}>
                {validSelectedStudents.length === selectablePeriodEnrollmentCount && periodEnrollmentStudents.length ? 'Quitar todos' : 'Seleccionar todos'}
              </button>
            </div>
            <p>
              {validSelectedStudents.length} de {periodEnrollmentStudents.length} estudiante(s) validado(s) del periodo origen seleccionados para matricular en el periodo destino.
              {excludedCareerRows ? ` Se excluyeron ${excludedCareerRows} registro(s) de Inglés.` : ''}
              {duplicatedRows ? ` Se unificaron ${duplicatedRows} registro(s) duplicado(s).` : ''}
            </p>
            <div className="practicas-student-list">
              {periodEnrollmentStudents.length ? periodEnrollmentStudents.map((student) => {
                ...
                <span className="portal-muted">Selecciona un periodo y carga estudiantes. La carrera de Inglés no se incluye en este proceso.</span>
              )}
            </div>
          </div>
