...
async def upload_senescyt_titles_bulk(
    current_user: Annotated[SessionUser, Depends(_ADMIN_ACCESS)],
    modelo: Annotated[str, Form()],
    observacion: Annotated[str, Form()] = "",
    ...
    one_drive_folder = "/".join(
        [
            _ONEDRIVE_ROOT_FOLDER,
            _ONEDRIVE_TYPE_FOLDERS["senescyt"],
            _safe_folder_name(clean_model, "modelo"),
        ]
    )
    batch_id = uuid4().hex
    ...
    for file in files:
        ...
        identifications = _senescyt_identifications_from_text(text)
        senescyt_code = _senescyt_registration_code_from_text(text)
        ...
        for cedula in valid_documents:
            ...
            item_id = uuid4().hex
            renamed_filename = _title_filename("SENESCYT", cedula, student.get("estudiante", ""), original_name, ".pdf", item_id)
            try:
    ...
    message = f"Carga masiva finalizada: {len(created_items)} título(s) registrado(s)."
