...
PROCESS_LABELS = {
    "PPF": "Prácticas laborales",
    "VIN": "Servicio Comunitario",
}
_EXCLUDED_PROCESS_CAREER_CODES = {"12"}
...
def _is_excluded_process_career_code(value: Any) -> bool:
    return _clean(value) in _EXCLUDED_PROCESS_CAREER_CODES


def _valid_process_career_condition(alias: str, code_column: str, career_column: str) -> str:
    prefix = f"{alias}." if alias else ""
    return (
        f"ISNULL(TRY_CONVERT(int, {prefix}{code_column}), -1) NOT IN (12) "
        f"AND UPPER(COALESCE(CONVERT(nvarchar(250), {prefix}{career_column}), N'')) NOT LIKE N'%INGL%'"
    )
...
def _build_carta_compromiso_pdf(expediente: Any) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=1.8 * cm,
        bottomMargin=1.7 * cm,
        title="Carta compromiso prácticas laborales",
    )
    story: list[Any] = [
        Paragraph("INSTITUTO SUPERIOR TECNOLÓGICO INTEC", title_style),
        Paragraph("CARTA COMPROMISO DE PRÁCTICAS LABORALES", title_style),
        Spacer(1, 0.2 * cm),
    ]
    table_data = [
    ...
    story.append(Paragraph(
        ...
        "académicas, administrativas y éticas asociadas al desarrollo de mis prácticas laborales.",
        body_style,
    ))
    story.append(Paragraph(
...
def catalog(_: Annotated[SessionUser, Depends(_ALL_ACCESS)]) -> dict[str, Any]:
    try:
        with get_practices_connection() as conn:
            ...
            if _use_legacy_schema(cursor):
                cursor.execute(
                    f"""
                    SELECT
                        tp.tipo_proceso_id AS TipoProcesoId,
                        tp.codigo AS Codigo,
                        tp.nombre AS Nombre,
                        tp.descripcion AS Descripcion,
                        tp.activo AS Activo
                    FROM cat.tipo_proceso tp
                    WHERE tp.activo = 1
                    ORDER BY tp.codigo
                    f"""
                )
        ...
        raise _db_error(exc, "No se pudo cargar el catálogo de Prácticas laborales y Servicio Comunitario") from exc
...
def student_practices(
    ...
    try:
        with get_practices_connection() as conn:
            ...
            if _use_legacy_schema(cursor):
                cursor.execute(
                    ...
                    WHERE codigo_estud = ?
                      AND ({_valid_process_career_condition("", "cod_anio_basica", "carrera")})
                    ORDER BY codigo_periodo DESC, cod_anio_basica, tipo_proceso_codigo
                    """,
                    student_code,
                )
            else:
                if _has_column(cursor, "integ.vw_estudiantes_elegibles", "TipoProcesoCodigo"):
                    cursor.execute(
                        f"""
                        SELECT TOP 100 *
                        FROM integ.vw_estudiantes_elegibles
                        WHERE codigo_estud = ?
                          AND ({_valid_process_career_condition("", "CodigoCarrera", "Carrera")})
                    ...
                    cursor.execute(
                        ...
                        SELECT TOP 100
                            ...
                            v.NombrePeriodo,
                            tp.Codigo AS TipoProcesoCodigo,
                            CASE WHEN tp.Codigo = 'VIN' THEN N'Servicio Comunitario' ELSE N'Prácticas laborales' END AS TipoProceso,
                            v.SemestreMaximo,
                            v.EsElegible
                        FROM integ.vw_estudiantes_elegibles v
                        CROSS JOIN cat.TipoProceso tp
                        WHERE v.codigo_estud = ?
                          AND ({_valid_process_career_condition("v", "CodigoCarrera", "Carrera")})
                          AND tp.Activo = 1
                          AND tp.Codigo IN ('PPF', 'VIN')
                        ORDER BY v.CodigoPeriodo DESC, v.CodigoCarrera, tp.Codigo
                        """,
                        student_code,
                    )
        ...
        raise _db_error(exc, "No se pudo consultar Prácticas laborales y Servicio Comunitario del estudiante") from exc
...
def create_student_expediente(
    ...
    if _is_excluded_process_career_code(payload.codigo_carrera):
        raise HTTPException(status_code=400, detail="La carrera de Inglés no se matricula en este proceso. Selecciona la carrera académica del estudiante.")
    try:
        with get_practices_connection() as conn:
            cursor = conn.cursor()
            if _use_legacy_schema(cursor):
                ...
                estado_row = cursor.fetchone()
                if not estado_row:
                    raise HTTPException(status_code=500, detail="Faltan catálogos base de Prácticas laborales y Servicio Comunitario.")
                cursor.execute("SELECT ISNULL(MAX(expediente_id), 0) + 1 FROM pp.expediente_practica WITH (UPDLOCK, HOLDLOCK)")
                next_id = int(cursor.fetchone()[0])
                code = f"{process_code}-{next_id:08d}"
                cursor.execute(
    ...
    try:
        with get_practices_connection() as conn:
            ...
            if not _use_legacy_schema(cursor):
                raise HTTPException(status_code=400, detail="La generación de carta está disponible para la estructura actual de Prácticas laborales.")
            ...
            if _clean(expediente.tipo_proceso_codigo).upper() != "PPF":
                raise HTTPException(status_code=400, detail="La carta compromiso aplica solo para Prácticas laborales.")
...
async def upload_carta_compromiso(
    ...
    try:
        with get_practices_connection() as conn:
            cursor = conn.cursor()
            if not _use_legacy_schema(cursor):
                raise HTTPException(status_code=400, detail="La carga de carta está disponible para la estructura actual de Prácticas laborales.")
            expediente = _fetch_legacy_expediente(cursor, expediente_id)
            if not expediente:
                raise HTTPException(status_code=404, detail="Expediente no encontrado.")
            _ensure_student_owns_expediente(current_user, expediente)
...
async def upload_certificado_preprofesional(
    ...
    try:
        with get_practices_connection() as conn:
            cursor = conn.cursor()
            if not _use_legacy_schema(cursor):
                raise HTTPException(status_code=400, detail="La carga de certificado está disponible para la estructura actual de Prácticas laborales.")
            expediente = _fetch_legacy_expediente(cursor, expediente_id)
            if not expediente:
                raise HTTPException(status_code=404, detail="Expediente no encontrado.")
            _ensure_student_owns_expediente(current_user, expediente)
            if _clean(expediente.tipo_proceso_codigo).upper() != "PPF":
                raise HTTPException(status_code=400, detail="El certificado aplica solo para Prácticas laborales.")
            ...
            if _use_legacy_schema(cursor):
                ...
                cursor.execute(
                    ...
                    WHERE v.tipo_proceso_codigo = ?
                      ...
                      AND ({_valid_process_career_condition("v", "cod_anio_basica", "carrera")})
                      AND (
                            ? = '%%'
                ...
                if _has_column(cursor, "integ.vw_estudiantes_elegibles", "TipoProcesoCodigo"):
                    ...
                    cursor.execute(
                        ...
                        SELECT TOP ({limit})
                            ...
                            v.SemestreMaximo,
                            v.EsElegible,
                            CASE WHEN v.EsElegible = 1 THEN N'Elegible' ELSE N'Pendiente' END AS MotivoElegibilidad,
                            CAST(0 AS bit) AS TieneAutorizacion,
                            CAST(NULL AS bigint) AS AutorizacionId,
                            CAST(NULL AS nvarchar(260)) AS AutorizacionArchivo,
                            CAST(NULL AS nvarchar(500)) AS AutorizacionUrl,
                            CAST(NULL AS datetime2) AS AutorizacionFecha,
                            v.EsElegible AS PuedeMatricular
                        FROM integ.vw_estudiantes_elegibles v
                        INNER JOIN cat.TipoProceso tp ON tp.Codigo = ? AND tp.Activo = 1
                        WHERE v.EsElegible = 1
                          AND ({_valid_process_career_condition("v", "CodigoCarrera", "Carrera")})
                          AND (? = '' OR CONVERT(varchar(50), v.CodigoPeriodo) = ?)
                          AND (
                                ? = '%%'
                             OR v.Apellidos_nombre LIKE ?
                             OR v.Cedula_Est LIKE ?
                             OR v.Carrera LIKE ?
                             OR v.NombrePeriodo LIKE ?
                             OR CONVERT(varchar(50), v.codigo_estud) LIKE ?
                          )
                        ORDER BY v.NombrePeriodo DESC, v.Apellidos_nombre
                        """,
                        process,
                        period,
                        period,
...
def admin_periods(
    ...
    try:
            ...
            if _has_object(cursor, "pp.vw_estudiantes_elegibles_proceso"):
                ...
                if _has_column(cursor, "integ.vw_estudiantes_elegibles", "TipoProcesoCodigo"):
                    cursor.execute(
                        f"""
                        SELECT
                            CONVERT(varchar(50), CodigoPeriodo) AS CodigoPeriodo,
                            COUNT(DISTINCT codigo_estud) AS TotalEstudiantes
                        FROM integ.vw_estudiantes_elegibles
                        WHERE TipoProcesoCodigo = ?
                          AND ({_valid_process_career_condition("", "CodigoCarrera", "Carrera")})
                    ...
                    cursor.execute(
                        ...
                        SELECT
                            ...
                            COUNT(DISTINCT codigo_estud) AS TotalEstudiantes
                        FROM integ.vw_estudiantes_elegibles
                        WHERE {_valid_process_career_condition("", "CodigoCarrera", "Carrera")}
                        GROUP BY CodigoPeriodo
                        """
                    )
                counts = {
        ...
        raise _db_error(exc, "No se pudo consultar periodos del proceso") from exc
...
async def upload_admin_authorization(
    ...
    try:
        with get_practices_connection() as conn:
            cursor = conn.cursor()
            if not _use_legacy_schema(cursor):
                raise HTTPException(status_code=400, detail="La autorización está disponible para la estructura actual de Prácticas laborales.")
            _ensure_period_designation_table(cursor)
            ...
            cursor.execute(
                ...
                digest,
                len(content),
                "Autorización administrativa para habilitar Prácticas laborales antes de tercer semestre.",
                current_user.login,
            )
            row = cursor.fetchone()
            conn.commit()
...
def save_period_designation(
    ...
    try:
        with get_practices_connection() as conn:
            cursor = conn.cursor()
            if not _use_legacy_schema(cursor):
                raise HTTPException(status_code=400, detail="La designación por periodo está disponible para la estructura actual de Prácticas laborales y Servicio Comunitario.")
            _ensure_period_designation_table(cursor)
            tipo_id = _tipo_proceso_id(cursor, process)
            ...
            cursor.execute(
                f"""
                WITH fuentes_validas AS (
                    SELECT
                        v.*,
                        ROW_NUMBER() OVER (
                            PARTITION BY TRY_CONVERT(decimal(18,0), v.codigo_estud)
                            ORDER BY
                                CASE WHEN v.elegible = 1 THEN 0 ELSE 1 END,
                                TRY_CONVERT(decimal(18,0), v.cod_anio_basica)
                        ) AS FilaProceso
                    FROM pp.vw_estudiantes_elegibles_proceso v
                    INNER JOIN cat.tipo_proceso tp ON tp.codigo = v.tipo_proceso_codigo
                    OUTER APPLY (
            ...
            if missing_students:
                raise HTTPException(
                    status_code=400,
                    detail=f"Estudiantes no encontrados, sin carrera válida del proceso, no están en tercer semestre o no tienen autorización cargada: {', '.join(map(str, missing_students))}",
                )
        ...
        return {
            "ok": True,
            "message": "Matrícula por periodo registrada correctamente.",
